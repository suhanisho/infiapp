...
class GeneratedDynamoDBHelpersTest(unittest.TestCase):
    def test_put_get_delete_sample_messages(self) -> None:
        ...
        item = get_sample_messages("infiapp", "message-1", dynamodb_resource=dynamodb)
        self.assertEqual(item["message"] if item else None, "hello")
    ...
    def test_query_sample_messages_passes_query_options(self) -> None:
        dynamodb = FakeDynamoDBResource()
        put_sample_messages(
            {
                "app_name": "infiapp",
