...
def render_mock_agents_ts(agents: list[dict[str, Any]]) -> str:
    ...
    for agent in sorted(external_agents, key=lambda item: item["name"]):
        ...
        store_output = f"{type_prefix(agent['name'], 'store_message')}Output"
        lines.extend(
            [
                f"const {store_name}: NonNullable<{store_output}['item']>[] = [];",
                "",
                ...
                f"    const offset = typeof payload.nextMessageId === \"string\" ? {store_name}.findIndex((item) => item.messageId === payload.nextMessageId) + 1 : 0;",
                "    const start = Math.max(0, offset);",
                f"    const messages = {store_name}.slice(start, start + limit);",
                "    const lastMessage = messages[messages.length - 1];",
                f"    const hasMore = start + messages.length < {store_name}.length;",
                "    return {",
                "      messages,",
